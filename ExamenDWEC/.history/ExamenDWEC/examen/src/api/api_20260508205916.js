...
export async function setFavoriteGame(gameId) {
  const response = await fetch(`${API_URL}/me/favorite-game`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json'
    },
  ...
  if (!response.ok || !result.success) {
    throw new Error(result.message || 'Error al actualizar el videojuego favorito')
  }
