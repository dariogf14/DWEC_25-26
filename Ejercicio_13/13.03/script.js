...
const gradeEnglish = document.getElementById('gradeEnglish');
const gradeArt = document.getElementById('gradeArt');

let currentUserForGrades = null;
    ...
    users
        ...
        .forEach(user => {
            ...
            if(user.calificaciones){
                gradesHTML = `
                    M: ${user.calificaciones.Matemáticas}<br>
                    H: ${user.calificaciones.Historia}<br>
                    C: ${user.calificaciones.Ciencia}<br>
                    I: ${user.calificaciones.Inglés}<br>
                    A: ${user.calificaciones.Arte}
                `;
            }
            tr.innerHTML = `
                ...
                <td>
                    ...
                    <button class="grades-btn" data-id="${user._id}">Calificaciones</button>
                </td>
            `;
...
usersTableBody.addEventListener('click', async (e) => {
    ...
    } else if (e.target.classList.contains('grades-btn')) {
        const user = usersCache.find(u => u._id === id);
        openGradesModal(user);
    }
});
...
function openGradesModal(user) {
    currentUserForGrades = user;
    modalTitle.textContent = `Calificaciones de ${user.firstName} ${user.lastName}`;
    
    const grades = user.calificaciones || {};
    gradeMath.value = grades.Matemáticas ?? '';
    gradeHistory.value = grades.Historia ?? '';
    gradeScience.value = grades.Ciencia ?? '';
    gradeEnglish.value = grades.Inglés ?? '';
    gradeArt.value = grades.Arte ?? '';

    gradesModal.style.display = 'block';
}

closeModal.onclick = () => { gradesModal.style.display = 'none'; };
window.onclick = (e) => { if(e.target === gradesModal) gradesModal.style.display = 'none'; };

gradesForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    if (!currentUserForGrades) return;

    const newGrades = {
        Matemáticas: parseFloat(gradeMath.value),
        Historia: parseFloat(gradeHistory.value),
        Ciencia: parseFloat(gradeScience.value),
        Inglés: parseFloat(gradeEnglish.value),
        Arte: parseFloat(gradeArt.value)
    };

    for (const key in newGrades) {
        const val = newGrades[key];
        if (isNaN(val) || val < 0 || val > 10) {
            showMessage(`Calificación inválida en ${key}`, 'error');
            return;
        }
    }

    const updatedUser = {...currentUserForGrades, calificaciones: newGrades};

    try {
        showLoading(true);
        await fetch(`${API_URL}/${currentUserForGrades._id}`, {
            method: 'PUT',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({
                ...
                calificaciones: updatedUser.calificaciones
            })
        });
        usersCache = usersCache.map(u => u._id === updatedUser._id ? updatedUser : u);
        displayUsers(usersCache);
        showMessage('Calificaciones guardadas correctamente');
...
deleteGradesBtn.addEventListener('click', async () => {
    if (!currentUserForGrades) return;
    if (!confirm('¿Deseas eliminar todas las calificaciones?')) return;

    const updatedUser = {...currentUserForGrades};
    delete updatedUser.calificaciones;
    ...
    try {
        ...
        usersCache = usersCache.map(u => u._id === updatedUser._id ? updatedUser : u);
        displayUsers(usersCache);
        showMessage('Calificaciones eliminadas correctamente');
        gradesModal.style.display = 'none';
    } catch (err) {
        showMessage('Error al eliminar calificaciones', 'error');
        console.error(err);
    } finally {
        showLoading(false);
    }
